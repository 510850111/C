...
void gotoxy(HANDLE hOut, int x, int y)
{
 COORD pos;//定义一个控制光标的pos
 pos.X = x;  //横坐标
 pos.Y = y;  //纵坐标
 SetConsoleCursorPosition(hOut, pos);//把光标移动到x,y处.
}
/******制作游戏窗口******************************************************************/
void make_frame()
{
 ...
 printf("************");
 gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+8);
 printf("下一个方块：");
...
void print_tetris(HANDLE hOut,struct Tetris *tetris)
{
 ...
 printf("等级 : %d",tetris->level);
 gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+3);
 printf("得分 : %d",tetris->score);
 gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+5);
 printf("下落速度 : %dms/一格",tetris->speed);
}
/******清除俄罗斯方块的痕迹****************************************************************/
